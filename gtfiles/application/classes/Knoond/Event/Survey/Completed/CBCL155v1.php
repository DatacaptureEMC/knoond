 ...
 * Versie 1 - juni 2018
 *
 * Script voor de CBCL/1½-5
 * Dit script haalt T-scores op uit de CBCL-stamtabel voor de 5 DSM-Schalen, 7 Syndroomschalen, 
 * Schaal Internaliseren, Schaal Externaliseren en de Totaalscore.
 * Vervolgens wordt bepaald in welk gebied de T-score zich bevind; Normaal, Borderline of Klinisch
 */

class Knoond_Event_Survey_Completed_CBCL155v1 extends \EMC_Event_SurveyCompletedEventAbstract {   
    /**
	...
	//Patient gegevens
        ...
        $AgeMonths = $token->getRespondent()->getAgeMonths($token->getCompletionTime());
        $AgeComment = '';
        $Gender = $token->getRespondent()->getGender();

        if ($AgeMonths < 18) {
            $Age = 1;
            $AgeComment = "Patient is jonger dan 1,5 jaar: normering voor 1,5-5 jarigen toegepast.";
        } elseif ($Age > 5) {
            $Age = 5;
            $AgeComment = "Patient is ouder dan 5 jaar: normering voor 1,5-5 jarigen toegepast.";
        }
        // Schaalscores:
	...
	// Exporteer resultaten naar LimeSurvey:
	...
	$results ['stamtabelVersie'] = $stamtabelVersie;
        ...
        /**
         * Het stuk hier onder laat je ook altijd staan
         */
